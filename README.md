...
1. **Option A: Gmail Integration (Recommended)**
   ...
   - **Daily sync automatically processes Gmail payments** - no manual intervention needed
   - No Zapier setup required - payment emails from Blawby are automatically detected and processed
   - Test with **"Process Gmail Payments"** menu item

2. **Option B: Zapier Integration (Alternative)**
...
Blawby includes **built-in Gmail payment detection** as the **primary recommended approach** for payment processing. This feature automatically monitors your Gmail for payment notification emails from Blawby and processes them seamlessly as part of the daily sync workflow.

### **How Gmail Integration Works**

1. **Automatic Monitoring** - Checks Gmail every 15 minutes for payment notifications
2. **Daily Sync Integration** - Payment processing is automatically included in daily sync operations
3. **Smart Parsing** - Extracts payment amount, client email, payment method, and unique payment ID
4. **Duplicate Prevention** - Uses Message-ID to prevent processing the same payment twice
5. **Seamless Processing** - Automatically triggers full client sync and balance calculations
6. **Email Management** - Marks processed emails as read and archives them
